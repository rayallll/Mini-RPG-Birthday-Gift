...
//=============================================================================
 /*:
 * @plugindesc 【YEP❀实用类】图片触发公共事件|YEP_PictureCommonEvents.js
 ...
 * @parent ---Picture 1---
 * @type common_event
 * @desc 在持有具有此 ID 的图片时运行此常见事件
 * 每 6 帧。将此值保留为 0 表示无常见事件。
 * @default 0
 *
 ...
 *
 * 图片可以作为游戏视觉效果以外的其他功能。
 * 它们有时可以用作UI元素或更多！但是，他们没有
 * 具有固有的能力就是在点击时调用常见事件
 * 通过鼠标打开。使用此插件，绑定到常见事件的图片将
 * 点击或触摸时变得可用！
 *
 * ============================================================================
 * ▼ 指示
 * ============================================================================
 *
 * 在插件的参数中，您可以为每个图片 ID 设置公共事件 ID
 * 从 1 到 100 列出（RPG Maker MV 中使用的最大图片数量为 100）。
 * 如果图片的值为1不透明度，则可以使用该图片
 * 对于普通活动。有四种条件类型可以绑定
 * 常见事件：
 *
 * ---
 ...
 * -点击-
 * 如果点击图片，这将提示触发公共事件
 * 如果列出的常见事件值高于 0。这将仅触发一次
 * 直到玩家释放鼠标（或停止触摸屏幕）。
 *
 * -重复-
 * 如果单击然后按住鼠标（或者如果触摸屏被按下并且
 * hold），只要鼠标在图片区域上方和公共事件
 * 列出的值高于 0，则每 6 帧触发一次，直到
 * 播放器释放鼠标（或停止触摸屏幕）或移动
 * 鼠标超出图片边界。
 *
 * - 按住-
 * 如果单击然后按住鼠标（或者如果触摸屏被按下并且
 * hold），只要鼠标在图片区域上方和公共事件
 * 列出的值高于 0，那么这将触发每一帧，直到播放器
 * 释放鼠标（或停止触摸屏幕）或将鼠标移出
 * 的图片边界。
 *
 * -释放-
 * 释放点击的鼠标时（或玩家停止触摸的那一刻）
 * 屏幕）并释放此图片上方的鼠标，如果图片的
 * 列出的常见事件值高于 0，这将在发布时触发。
 *
 * ---
 * 
 * 请注意，绑定到的图片有一些行为变化
 * 常见事件。绑定到常见事件的任何图片都将是
 * 与地图的精灵集分开，而是绑定到场景。这是
 * 便在屏幕滚动，放大，摇晃等情况下，它
 * 不会影响绑定的图片。
 *
 * 这是因为图片通常用作UI元素和UI
 ...
 *   EnableTouchMove
 *   DisableTouchMove
 *   - 允许玩家按下屏幕上的目的地并移动到那里或
 *   禁用播放器使用触摸移动。
 *   
 *   HidePictureCommonEvents
 *   ShowPictureCommonEvents
 *   - 手动隐藏绑定到常见事件的所有图片或显示它们。如果显示
 *  然后图片将恢复到之前的不透明度值。隐藏图片
 *  无法触发激活常见事件。
 *
 *   MovePlayer Down
 *   MovePlayer Left
 *   MovePlayer Right
 *   MovePlayer Up
 *   - 将导致玩家角色移动，就像使用 4 目录移动一样
 *   输入（通过键盘或控制器）。这最好与“按”一起使用。
 *
 *   TriggerButton Ok
